    ...
    <>
      <header
        className={`fixed top-0 w-full z-[1000] transition-all duration-500 ${
          scrolled
            ? "bg-[#1a1a1a]/95 backdrop-blur-md h-20 border-b border-white/10 shadow-xl"
            : "bg-transparent h-24"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
          ...
          >
            ...
            <div className="hidden sm:flex flex-col items-start">
              <span className="text-xl md:text-2xl font-pistilli leading-none tracking-tighter text-white uppercase">
                ESTACIÓN 9 3/4
              </span>
              <span className="text-[10px] uppercase tracking-[0.4em] text-gray-400 font-industrial mt-1">
                La Plata
              </span>
            </div>
          </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 z-[999] bg-zinc-950/98 backdrop-blur-2xl transition-all duration-500 lg:hidden ${
          isMenuOpen
        ...
        <div className="flex flex-col items-center justify-center h-full space-y-12">
          {navLinks.map((link, index) => (
            <a
              ...
              href={link.href}
              onClick={(e) => handleLinkClick(e, link.href)}
              className={`text-4xl font-pistilli text-white uppercase italic tracking-tighter transition-all duration-700 transform ${
                isMenuOpen
