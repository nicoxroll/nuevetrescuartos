...
const FloatingChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "¡Saludos! Soy el Guardián del Andén. Nuestras hamburguesas están saliendo más calientes que el aliento de un dragón. ¿En qué puedo ayudarte hoy?",
    },
  ]);
    ...
    >
      ...
      >
        {isOpen ? (
          ...
          <MagicIcon className="w-8 h-8 text-black" />
        )}
      </button>

      <div
        className={`absolute bottom-20 right-0 w-[85vw] sm:w-[380px] h-[60vh] max-h-[450px] bg-zinc-950/98 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl flex flex-col transition-all duration-500 origin-bottom-right ${
          isOpen
      ...
      >
        <div className="p-5 border-b border-white/5 flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-pistilli text-white uppercase leading-none">
              El Guardián
            </h3>
            <span className="text-[7px] uppercase tracking-[0.3em] text-gray-500 font-industrial">
              Soporte Mágico
            </span>
          </div>
        </div>
