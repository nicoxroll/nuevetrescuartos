      ...
      <div className="relative w-full max-w-2xl max-h-[90dvh] bg-zinc-950 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col">
        ...
        <div className="flex flex-col md:flex-row h-full overflow-hidden">
          <div className="w-full md:w-1/2 flex-shrink-0 h-48 md:h-auto border-b md:border-b-0 md:border-r border-white/10">
            <img src={product.image} alt={product.name} className="w-full h-full object-cover grayscale-[0.3]" />
          </div>
          <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto">
            <div className="flex-grow space-y-6">
              <div>
                <h2 className="text-3xl md:text-4xl font-pistilli text-white uppercase italic mb-1 tracking-tight">{product.name}</h2>
                <p className="text-xl md:text-2xl font-industrial text-gray-400 tracking-wider">$ {product.price}</p>
              </div>
              <p className="text-gray-500 font-serif italic leading-relaxed text-sm md:text-base">
                {product.description}
              </p>
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Aclaraciones / Notas</label>
                <textarea 
                  value={note}
