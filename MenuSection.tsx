...
const MenuSection: React.FC<MenuSectionProps> = ({ products, selectedCategory, onSelectCategory, onOpenProduct, onAddToCartDirectly }) => {
  const categories: (Category | 'all')[] = ['all', 'burgers', 'sides', 'drinks', 'desserts'];
    ...
    <section id="menu" className="relative py-24 bg-[#0c0c0c]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-20">
          <h2 className="text-6xl md:text-9xl font-pistilli mb-8 text-white uppercase italic tracking-tighter">Carta</h2>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {categories.map((cat) => (
              ...
              >
                {cat === 'all' ? 'Ver Todo' : cat === 'burgers' ? 'Burgers' : cat === 'sides' ? 'Sides' : cat === 'drinks' ? 'Drinks' : 'Sweets'}
              </button>
            ))}
          </div>
        </div>
          ...
          {products.map(item => (
              ...
              <div className="flex-grow flex flex-col py-2 border-b border-white/5">
                <div className="flex justify-between items-baseline mb-4">
                  <h4 className="text-2xl font-pistilli uppercase tracking-tight text-white group-hover:text-gray-300 transition-colors">{item.name}</h4>
                  <span className="text-xl font-industrial text-gray-400">$ {item.price}</span>
                </div>
